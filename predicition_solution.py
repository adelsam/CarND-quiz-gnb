...
def run():
    gnb = GNB()
    states = open("nd013_pred_data/train_states.txt", "r")
    states = [[float(x) for x in s.strip().split(",")] for s in states]
    labels = open("nd013_pred_data/train_labels.txt", "r")
    labels = [s.strip() for s in labels]
    gnb.train(states, labels)

    states_test = open("nd013_pred_data/test_states.txt", "r")
    states_test = [[float(x) for x in s.strip().split(",")] for s in states_test]
    labels_test = open("nd013_pred_data/test_labels.txt", "r")
    labels_test = [s.strip() for s in labels_test]

    result = []
    for x_test, label in zip(states_test, labels_test):
        prediction = gnb.predict(x_test)
        print(str(x_test) + " actual: " + label + " predicted: " +
              prediction)
    ...
    print("Correct %: " + str(float(correct_count)/len(result)))
